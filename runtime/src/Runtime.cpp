    ...
    void Runtime::f_updateScripts(RType::Runtime::ECS::Entity entity)
    {
        ...
        SKIP_EXCEPTIONS({
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (isServer()) {
                    LuaApi::ExecFunction(m_lua, getProjectPath() + "/assets/scripts/" + script.paths[i], "updateServer",
                                         entity);
                    f_updateColliders(entity, script.paths[i]);
                } else {
                    LuaApi::ExecFunction(m_lua, getProjectPath() + "/assets/scripts/" + script.paths[i], "update",
                                         entity);
                }
            }
        })
        ...
        SKIP_EXCEPTIONS({
            ...
            if (!isServer())
                return;
            LuaApi::ExecFunction(m_lua, getProjectPath() + "/assets/scripts/" + controllable.scriptPath, "updateServer",
                                 entity);
